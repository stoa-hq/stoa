...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
	...
	<div class="mb-8">
		...
		<p class="text-gray-500 mt-1">{$t('products.itemCount', { values: { count: meta?.total ?? 0 } })}</p>
	</div>
	...
	<div class="flex flex-col lg:flex-row gap-8">
		...
		<aside class="lg:w-56 flex-shrink-0">
			...
			{#if categories.length > 0}
				<div>
					<h2 class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">{$t('products.categories')}</h2>
					<ul class="space-y-1">
						<li>
							<button
