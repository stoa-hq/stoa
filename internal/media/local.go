...
// mediaPathRe matches paths generated by Store(): YYYY/MM/xxxxxxxx-filename
var mediaPathRe = regexp.MustCompile(`^\d{4}/\d{2}/[0-9a-f]{8}-.+$`)
...
func (s *LocalStorage) Delete(ctx context.Context, path string) error {
	fullPath, err := validateMediaPath(s.basePath, path)
	if err != nil {
		return err
	}
...
func validateMediaPath(basePath, relPath string) (string, error) {
	if !mediaPathRe.MatchString(relPath) {
		return "", fmt.Errorf("invalid media path format: %q", relPath)
	}
	fullPath := filepath.Join(basePath, filepath.Clean(relPath))
