<script lang="ts">
  ...
  function optionName(o: PropertyOption): string {
    return o.translations?.find((t) => t.locale === DEFAULT_LOCALE)?.name
  ...
  onMount(async () => {
    try {
      ...
      notifications.error('Eigenschaftsgruppe konnte nicht geladen werden.');
    } finally {
      loading = false;
    }
  });
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    try {
      ...
      notifications.success('Eigenschaftsgruppe gespeichert.');
    } catch {
  ...
  async function handleDeleteGroup() {
    try {
      ...
      notifications.success('Eigenschaftsgruppe gelöscht.');
  ...
  async function handleAddOption(e: SubmitEvent) {
    ...
    try {
      ...
      notifications.success('Ausprägung hinzugefügt.');
    } catch {
      notifications.error('Ausprägung konnte nicht erstellt werden.');
    } finally {
      optSubmitting = false;
    }
  }
  ...
  async function handleUpdateOption(e: SubmitEvent) {
    ...
    try {
      ...
      editOption = null;
      notifications.success('Ausprägung gespeichert.');
    } catch {
      notifications.error('Speichern fehlgeschlagen.');
    } finally {
  ...
  async function handleDeleteOption() {
    ...
    try {
      ...
      deleteOptionId = null;
      notifications.success('Ausprägung gelöscht.');
    } catch {
      notifications.error('Löschen fehlgeschlagen.');
    }
  }
...
{:else}
  ...
  <div class="card p-6 max-w-lg mb-6">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-xl font-bold text-gray-900">Eigenschaftsgruppe bearbeiten</h1>
      <button class="btn btn-danger btn-sm" onclick={() => showDeleteGroupConfirm = true}>Löschen</button>
    </div>
  ...
  <div class="card p-6 max-w-lg">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-lg font-semibold text-gray-900">Ausprägungen</h2>
    ...
    {#if options.length === 0}
      <p class="text-sm text-gray-400">Noch keine Ausprägungen vorhanden.</p>
    {:else}
...
<ConfirmModal
  ...
  title="Eigenschaftsgruppe löschen"
  message="Soll diese Eigenschaftsgruppe wirklich gelöscht werden? Alle zugehörigen Ausprägungen werden ebenfalls gelöscht."
...
<ConfirmModal
  open={!!deleteOptionId}
  title="Ausprägung löschen"
  message="Soll diese Ausprägung wirklich gelöscht werden?"
  onConfirm={handleDeleteOption}
  onCancel={() => deleteOptionId = null}
/>

<!-- Ausprägung hinzufügen -->
<Modal open={showAddOptionModal} title="Ausprägung hinzufügen" onClose={() => showAddOptionModal = false}>
  <form onsubmit={handleAddOption} class="space-y-4">
    ...
    <div>
      <label class="label" for="opt-color">Farbe (Hex)</label>
    ...
    <div>
      <label class="label" for="opt-position">Position</label>
    ...
    <div class="flex gap-3 pt-2">
      <button type="submit" class="btn btn-primary" disabled={optSubmitting}>
        {optSubmitting ? 'Speichern...' : 'Hinzufügen'}
      </button>
      <button type="button" class="btn btn-secondary" onclick={() => showAddOptionModal = false}>Abbrechen</button>
    </div>
  </form>
</Modal>
...
<!-- Ausprägung bearbeiten -->
{#if editOption}
  <Modal open={true} title="Ausprägung bearbeiten" onClose={() => editOption = null}>
    <form onsubmit={handleUpdateOption} class="space-y-4">
      ...
      <div>
        <label class="label" for="edit-opt-color">Farbe (Hex)</label>
        <div class="flex gap-2 items-center">
      ...
      <div>
        <label class="label" for="edit-opt-position">Position</label>
      ...
      <div class="flex gap-3 pt-2">
        <button type="submit" class="btn btn-primary" disabled={editOptSubmitting}>
          {editOptSubmitting ? 'Speichern...' : 'Speichern'}
        </button>
        <button type="button" class="btn btn-secondary" onclick={() => editOption = null}>Abbrechen</button>
      </div>
    </form>
  </Modal>
{/if}
